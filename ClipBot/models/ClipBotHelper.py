...
class ClipBotHelper(object):
    ...
    def processComments(self, comments):
        ...
        if os.path.exists(f"{self._pathName}/markov_chain.json"):
            with open(f"{self._pathName}/markov_chain.json", "r") as ifile:
                chain = load(ifile)
        else:
            chain = {}
                ...
                for prevWord in prevWords:
                    for i in range(len(words)):
                        if words[i] not in chain[prevWord]:
                            ...
                            chain[prevWord][words[i]] += 1
            prevWords = words
        with open(f"{self._pathName}/markov_chain.json", "w") as ofile:
